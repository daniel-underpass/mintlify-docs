---
title: "MCP server"
description: "Drop Luxxon into Claude Desktop, Cursor, Claude Code, or any MCP-aware agent. Ten tools, one npx command — open live video sessions at any lat/lng and feed JPEG frames straight to a vision model."
icon: "plug"
...
<Tabs>
  ...
  Restart Claude Desktop. The ten Luxxon tools appear under the
...
The first call right after `request_live_view` resolves may return
`FRAME_NOT_AVAILABLE` for 3-5s while Cloudflare's edge sends the
first keyframe. Retry with a short backoff.
